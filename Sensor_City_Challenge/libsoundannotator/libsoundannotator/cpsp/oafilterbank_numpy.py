...
class OAFilterbank(processor.Processor):
    ...
    def __init__(self,boardConn, name,*args, **kwargs):
        """ 
            ...
            hf = vector of filter coefficients

            hf can be a 2D vector in which case the first dimension is
    ...
    def initialize(self):
        hf=self.hf
        self.responseLength = np.shape(hf)[0]
...
class Resampler(OAFilterbank):
    ...
    def __init__(self,boardConn, name, *args, **kwargs):
        ...
        self.hf = sig.firwin(self.config['FilterLength'],normalizedCutoff,None,('kaiser',self.config['KaiserBeta']))
    ...
    def processData(self, inputs):
        ...
        if(self.openResampler):
            ...
            if not soundData==None:
                ...
                lowpassFiltered = self.oafilter(soundData.data)
