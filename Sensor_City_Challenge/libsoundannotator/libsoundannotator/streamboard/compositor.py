    ...
    def alignIncomingChunks(self,index, continuity, chunkcontinuity, starttime, dataGenerationTime,metadata, identifier):
        ...
        for key, current_chunk in current_composite.received.viewitems():
            ...
            if current_chunk.alignment.isAlignable():
                ...
                self.processor.logger.error("Keys in arriving chunk {0}".format(key))
                self.processor.logger.error("          lowindices_drop {0}".format(lowindices_drop))
                self.processor.logger.error("          highindices_drop {0}".format(highindices_drop))
                self.processor.logger.error("          chunkdiscontinuity_lowindicesdrop {0}".format(chunkdiscontinuity_lowindicesdrop))
                self.processor.logger.error("          dimension {0}".format(dimension))
