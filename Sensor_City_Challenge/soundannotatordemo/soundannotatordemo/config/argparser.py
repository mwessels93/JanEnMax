...
def settingsParse(parser, settings):
    ...
    # ... GUI and CHUI behaviour arguments
    behaviourgroup=parser.add_argument_group('GUI and CHUI behaviour')
    ...
    behaviourgroup.add_argument('--guimode',
        help='By default in "nogui" mode and the GUI is suppressed. Specify to bring up a GUI.',
        default='nogui',choices=['nogui','timescale_struct','timescale_patch','timescale_logE','timescale_pattern','esn_trace','soundtrace','li_trace'])

    #... input arguments
    inputgroup=parser.add_mutually_exclusive_group()
    inputgroup.add_argument('--node',nargs=3,type=str,
        help='Specify node id, ip address and port number',
        default=None)
    ...
    inputgroup.add_argument('-w','--wav',
        ...
        default=None)
    inputgroup.add_argument('--job',
        help='Start module that listens for audio file jobs',
        action='store_true')
    inputgroup.add_argument('-b', '--batch', type=str,
        help='Start batch file (.txt or CSV)',
        default=None)
    ...
    outputgroup.add_argument('--vEA',
        help='Output van Elburg-Andringa structured energy features',
        action='store_true')
    outputgroup.add_argument('--outdir',
        help='Output directory for audio data',
        type=str,
        default=os.path.join(os.path.expanduser("~"), "data", "libsoundannotator"))
    #outputgroup.add_argument('-p','--patches',
        #help='future extension',
        #action='store_true')

    networkgroup = parser.add_argument_group('Network properties')
    networkgroup.add_argument('--IP',
        help='Specify IP address for the network client and server to connect to',
        default=getArgument(settings, 'IP', '192.168.137.200')
    )
    networkgroup.add_argument('--port',
        type=int,
        help='Specify the port this Client uses to connect to the server',
        default=getArgument(settings, 'port', 65500)
    )

    servergroup = parser.add_argument_group('Server properties')
    servergroup.add_argument('--startport',
        type=int,
        help='Port to start listening to. Increments with number of clients to connect to',
        default=getArgument(settings, 'startport', 65500)
    )
    servergroup.add_argument('--endport',
        type=int,
        help='Port to end the listning. Defines the range to scan, together with startport',
        default=getArgument(settings, 'endport', 65510)
    )
    servergroup.add_argument('--clients',
        type=int,
        help='Number of clients to connect to. Defaults to 1',
        default=getArgument(settings, 'clients', 1)
    )
    ...
    testgroup.add_argument('--test',nargs=1,type=str,
        ...
        default=None)
    testgroup.add_argument('-f','--faststart',
        help='Initially several processors need to be calibrated, but the result is cached. With this option calibration is skipped.',
        action='store_true')
    ...
    ptnprocessorgroup.add_argument('--ptnreferencevalue', type=float,
        help='Set the blockwidth (in seconds) used for averaging P,T,N, and E ',
        default=getArgument(settings, 'ptnreferencevalue', 0) )
