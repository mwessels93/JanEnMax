...
from libsoundannotator.streamboard.processors.input   import noise, sine,  wav, sensorcity
...
class DummyProcessor(processor.OutputProcessor):
    requiredKeys = ["data"]
    ...
    def prerun(self):
        super(DummyProcessor, self).prerun()
    ...
    def processData(self, smartChunk):
        ...
        for key in self.requiredKeys:
            dataChunk= smartChunk.received[key]
            self.logger.info("Received {0}: {1} containing {2}".format(key, dataChunk, dataChunk.data))
...
def run():
   ...
   # Main should initialize logging for multiprocessing package
    ...
    if args.calibrate:
        ...
        if args.vEA:
            b.startProcessor('vEA',structuredEnergyProcessor.structuredEnergyProcessor,
                    ...
                    baseOutputDir=args.outdir,
                    globalOutputPathModifier=runtimeMetaData.outputPathModifier,
                    NumpyOut=True,MatlabOut=False,MatricesOut=False,
        ...
        if args.PTNE:
            ...
            #b.startProcessor("S2S_FileWriter-PTNE",DummyProcessor ,
    ...
    if args.guimode == 'timescale_logE':
        ...
        tfv=tf_plotter.tf_viewer(3*args.inputrate/(args.decimation*args.samplesperframe) , args.noofscales, toMyTFProcessor,logger, lowValue=0 , highValue=240, datakey='EdB',continuousBlockspacer=0)
        tfv.run()
